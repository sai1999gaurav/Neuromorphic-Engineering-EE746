function [L,grad_L]=energy_torus(cities,R, r, theta_z,k,l)
  ...
  A= 0.1; B = 0.4; C = 10; D = 7; F = 0;   % parameters
...
%     disp(f);
  ...
  for i=1:n
      ...
      grad_L(i,2) = B*((n*z0*imag((theta_z(i,2)/abs(theta_z(i,2)))^n))/(theta_z(i,2)')) - 2*A*complex(sin(theta(i,2)), cos(theta(i,2)))*((R - sqrt(coord_x(i).^2 + coord_y(i).^2))*(coord_x(i)*sin(theta(i,2)) - coord_y(i)*cos(theta(i,2)))/(sqrt(coord_x(i)^2 + coord_y(i)^2)));     
      ...
      grad_L(i,1) = -2*A*complex(sin(theta(i,1)), cos(theta(i,1)))*((R - sqrt(coord_x(i).^2 + coord_y(i).^2))*(coord_x(i)*sin(theta(i,1)*cos(theta(i,2)) + coord_y(i)*sin(theta(i,1))*sin(theta(i,2)) )/(sqrt(coord_x(i)^2 + coord_y(i)^2))) + coord_z(i)*cos(theta(i,1)));
      for j=1:n
